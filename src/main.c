    ...
    for (int i = 0; i < count; i++)
    {
        ...
        while (current != NULL)
        {
            ...
            case eBP_IMMEDIATE:
                ...
                break;
            case eBP_LABEL_ABS:
                printf("[%i:%i]subst : LABEL_ABS()", bit_elem->index_opcode, bit_elem->index_mnemonic, bit_elem->width);
                break;
            case eBP_LABEL_REL:
                printf("[%i:%i]subst : LABEL_REL()", bit_elem->index_opcode, bit_elem->index_mnemonic, bit_elem->width);
                break;
            case eBP_ENUM:
